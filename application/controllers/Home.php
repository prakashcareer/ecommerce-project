            ...
            if ($para1 == 'go') {
                if (isset($_POST['razorpay_payment_id'])) {
                    ...
                    try {
                            ...
                            $payment = $razorpay->payments->fetch($_POST['razorpay_payment_id']);
                            $payment->capture(array('amount' => $attributes['amount']));
                            // Payment successful
                    ...
                    if ($payment_success) { // Replace $payment_success with your Razorpay payment success condition
                        $this->crud_model->email_invoice($sale_id);

                        // Clear the cart
                        $this->cart->destroy();
                    
                        // Redirect to appropriate invoice page
                        if ($this->session->userdata('user_login') == 'yes') {
                            redirect(base_url() . 'home/invoice/' . $sale_id, 'refresh');
                        } else {
                            redirect(base_url() . 'home/guest_invoice/' . $data['guest_id'], 'refresh');
                        ...
                        // Payment unsuccessful, redirect back to cart checkout
                        $this->session->set_flashdata('alert', 'unsuccessful_razorpay');
                        redirect(base_url() . 'home/cart_checkout/', 'refresh');
                    }
                } else {
                    // No razorpay_payment_id received, redirect back to cart checkout
                    $this->session->set_flashdata('alert', 'unsuccessful_razorpay');
                    redirect(base_url() . 'home/cart_checkout/', 'refresh');
                }
            }
